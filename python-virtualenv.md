...
With Node, npm install the packages in a directory in your project (node_modules), which is how they can't leak. With Python, it installs dependencies in a common place, which means all your projects are sharing the same ones. A virtualenv lets you have multiple environments, which dependencies get installed into. That way, each project can have its own set of dependencies installed, without interfering with the others — just like how Node works
